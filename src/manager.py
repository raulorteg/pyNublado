...
class QueueManager:
    ...
    def __init__(self, target_dir: str = "data/sampes/sample_N4000",  ncpus: int = None, verbose: bool = True):
        ...
        # set number of cpus to use, if not defined use all available
        if not ncpus:
    ...
    def _run(self) -> None:
        ...
        model.in files using CLOUDY on as may CPUs as defined (user-defined maximum or system maximum)
        """
        pool = multiprocessing.Pool(processes=self.ncpus, maxtasksperchild=1)
